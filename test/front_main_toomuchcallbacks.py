...
def plot_mapbox(dateidx, language, city, type_inc, gun, hourclick, typeclick, replot):
    ...
    if "hour-chart" in changed_id and hourclick is not None:
        dfff = dff[(dff['hour'] == hourclick['points'][0]['label'])]
    elif "type-chart" in changed_id and typeclick is not None:
        dfff = dff[(dff['type'] == typeclick['points'][0]['label'])]
    elif "replot-all" in changed_id and replot>0:
        dfff = dff
    else:
        dfff = dff
...
def filter_tables_mapclick(dateidx, language, city, type_inc, gun, mapclick, hourclick, typeclick, replot):
    ...
    changed_id = [p['prop_id'] for p in dash.callback_context.triggered][0]
    if "main_graph" in changed_id and mapclick is not None:
        dfff=dff[ (dff['osm_lon'] == mapclick['points'][0]['lon'])&(dff['osm_lat'] == mapclick['points'][0]['lat'])]
    elif "hour-chart" in changed_id and hourclick is not None:
        dfff=dff[ (dff['hour'] == hourclick['points'][0]['label'])]
    elif "type-chart" in changed_id and typeclick is not None:
        dfff=dff[ (dff['type'] == typeclick['points'][0]['label'])]
    elif "replot-all" in changed_id and replot>0:
...
def update_type_chart(dateidx, language, city, type_inc, gun, hourclick, replot):
    ...
    fig = px.bar(dfff.head(20), y='incident counts', opacity=0.5)
...
def update_hour_chart(dateidx, language, city, type_inc, gun, typeclick, replot):
    ...
    changed_id = [p['prop_id'] for p in dash.callback_context.triggered][0]
    if "type-chart" in changed_id and typeclick is not None:
        dfff = dff[(dff['type'] == typeclick['points'][0]['label'])]
    elif "replot-all" in changed_id and replot>0:
        dfff = dff
    else:
    ...
    fig = px.bar(dfff, y='incident counts',opacity=0.5)
    fig.update_layout(height=300, width=500,
...
row_replot=html.Div([dbc.Row([
    dbc.Col(html.Div([
        # replot button
        dbc.Button('Refresh charts with top level filter settings', id='replot-all',
                    #title='refresh charts with top level filter settings',
        style={'color':'rgb(100,100,100)', 'background-color': 'rgb(256,256,256)', 'border': '0px'},
                    outline=False, block=True,
                    className="mr-1",
        n_clicks=0)
    ]), xl=4, style={"margin-top": "20px", "margin-bottom": "60px"}),
    ])
])
